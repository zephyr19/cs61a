...
def closer_city(lat, lon, city1, city2):
    ...
    closer = min(city1, city2, key=lambda city: distance(new_city, city))
...
def add_trees(t1, t2):
    ...
    new_branches = []
    if is_leaf(t1):
        new_branches = branches(t2)
    elif is_leaf(t2):
        new_branches = branches(t1)
    else:
        for b1, b2 in list(zip(branches(t1), branches(t2))):
            new_branches.append(add_trees(b1, b2))
        extra_branches = branches(max(t1, t2, key=lambda t: len(branches(t))))
        for index in range(len(new_branches), len(extra_branches)):
            new_branches.append(extra_branches[index])
