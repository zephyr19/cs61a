...
def count_change(amount):
    ...
    True
    """
    def partition_amount(amount, k):
        if amount == 0:
            return 1
        elif amount - pow(2, k) < 0:
            return 0
        else:
            with_k = partition_amount(amount-pow(2, k), k)
            without_k = partition_amount(amount, k+1)
            return with_k + without_k

    return partition_amount(amount, 0)
