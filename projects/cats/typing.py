...
def choose(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    new_strings = [x for x in paragraphs if select(x)]
    return new_strings[k] if k < len(new_strings) else ''
    # END PROBLEM 1
...
def accuracy(typed, reference):
    ...
    while i < min(len(typed_words), len(reference_words)):
        if lower(typed_words[i]) == lower(reference_words[i]):
            correct += 1
        i += 1
...
def swap_diff(start, goal, limit):
    ...
    is_diff = 0
    if start[0] != goal[0]:
        is_diff, limit = 1, limit - 1
    return is_diff + swap_diff(start[1:], goal[1:], limit)
    # END PROBLEM 6
...
def edit_diff(start, goal, limit):
    ...
    if limit < 0: # Fill in the condition
        ...
        add_diff = remove_diff = substitute_diff = edit_diff(start[1:], goal[1:], limit)
...
def fastest_words(word_times, margin=1e-5):
    ...
    assert margin > 0
    # BEGIN PROBLEM 9
    outcome = [[] for _ in range(n_players)]
    for i in range(n_words):
        all_time = [elapsed_time(word_times[j][i+1]) -
            elapsed_time(word_times[j][i]) for j in range(n_players)]
        min_time = min(all_time)
        min_index = [index for index, value in enumerate(all_time)
                            if value - min_time < margin]
        for index in min_index:
            outcome[index].append(word(word_times[0][i+1]))
    return outcome
    # END PROBLEM 9
