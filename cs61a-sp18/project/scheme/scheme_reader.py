...
def read_tail(src):
    ...
    try:
        if src.current() is None:
            ...
            # BEGIN PROBLEM 1
            src.remove_front()  # reomve ')'
            return nil
            # END PROBLEM 1
        elif src.current() == '.':
            # BEGIN PROBLEM 2
            src.remove_front()  # remove '.'
            val = src.remove_front()  # get the vale followed by the '.'
            if val == '(':
                val = read_tail(src)
            # check if there is a sencond item after '.'
            if src.current() != ')':
                raise SyntaxError('there should be only one item after the dot')
            else:
                src.remove_front()  # Remove ')'
                return val
            # END PROBLEM 2
