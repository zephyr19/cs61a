...
def trade(first, second):
    ...
    if can_change: # change this line!
...
def make_to_string(front, mid, back, empty_repr):
    ...
    def strings(lst):
        if lst is Link.empty:
            ...
            return front + str(lst.first) + mid \
                     + strings(lst.rest) + back
    return strings
...
def long_paths(tree, n):
    ...
    def generate_path(t):
        ...
        if t.is_leaf():
            return [Link(t.label)]
        for b in t.branches:
            for path in generate_path(b):
                paths.append(Link(t.label, path))
        return paths
    paths = generate_path(tree)
    return [path for path in paths if deep_len(path) > n]
# Orders of Growth
def zap(n):
    i, count = 1, 0
    while i <= n:
